...
// كائنات للبيانات سيتم جلبها من Backend
...
async function activateTool() {
    ...
    try {
        ...
        // جلب البيانات من Backend بعد التفعيل
        loadReportsData();
        loadToolsList();
        loadKeysFromStorage();
...
async function loadReportsData() {
    try {
        ...
        // جلب التصنيفات
        const categoriesResponse = await fetch(BACKEND_URL + "/reports/categories", {
        ...
        if (categoriesResponse.ok) {
            ...
            // تحديث خيار التصنيف في واجهة المستخدم
            ...
            // جلب جميع التقارير
            const allReportsResponse = await fetch(BACKEND_URL + "/reports/all", {
                headers: { "X-Activation-Code": activationCode }
            });
            
            if (allReportsResponse.ok) {
                const allReportsData = await allReportsResponse.json();
                allReports = allReportsData.reports;
                
                // بناء كائن التقارير مصنفة
                allReportsByCategory = {};
                categoriesData.categories.forEach(category => {
                    allReportsByCategory[category] = [];
                });
                
                allReports.forEach(report => {
                    if (allReportsByCategory[report.category]) {
                        allReportsByCategory[report.category].push(report.name);
                    }
                });
...
async function loadToolsList() {
    try {
        ...
        const response = await fetch(BACKEND_URL + "/tools/list", {
            headers: { "X-Activation-Code": activationCode }
        });
        
        if (response.ok) {
            const data = await response.json();
            console.log("الأدوات المتاحة:", data.tools);
        }
    } catch (error) {
        console.error("خطأ في تحميل قائمة الأدوات:", error);
    }
}
...
function handleReportSearch() {
    ...
    // إذا كان هناك تصنيف محدد وليس "أخرى"
    ...
    if (filteredReports.length > 0) {
        ...
        filteredReports.forEach(reportName => {
            const category = categorySelect.value || "التقارير التعليمية الصفية";
...
// دالة الذكاء الاصطناعي الأصلية المعدلة
async function fillWithAI() {
    ...
    try {
        // استخدام نقطة النهاية الجديدة في Backend
        const response = await fetch(BACKEND_URL + "/fill-with-ai", {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
        ...
        if (!data || !data.fields) {
            throw new Error('لم يتم الحصول على بيانات صحيحة من الخادم');
        }
        
        // تعبئة الحقول بالبيانات المستلمة
        const fields = data.fields;
        if (fields.goal) document.getElementById('goal').value = fields.goal;
        if (fields.summary) document.getElementById('summary').value = fields.summary;
        if (fields.steps) document.getElementById('steps').value = fields.steps;
        if (fields.strategies) document.getElementById('strategies').value = fields.strategies;
        if (fields.strengths) document.getElementById('strengths').value = fields.strengths;
        if (fields.improve) document.getElementById('improve').value = fields.improve;
        if (fields.recomm) document.getElementById('recomm').value = fields.recomm;
...
function updateManualDate() {
    const manualDate = document.getElementById('manualDateInput').value.trim();
    if (manualDate) {
        currentHijriDate = manualDate;
        document.getElementById('currentDateDisplay').textContent = `هجري: ${currentHijriDate}`;
        
        // تحويل التاريخ الهجري إلى ميلادي
        convertHijriToGregorian(manualDate).then(gregorianDate => {
